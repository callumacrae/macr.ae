...
export default function init() {
  ...
  const initChart = (selector, algo) => {
    ...
    chartUpdaters.push(newData => {
      ...
      rects
        ...
        .attr(
          'x',
          algoInstance && algoInstance.x
            ? algoInstance.x(xScale, width, data)
            : (d, i) => xScale(i)
        )
        .attr(
          'y',
          algoInstance && algoInstance.y
            ? algoInstance.y(yScale, height, data)
            : d => height - yScale(d)
        )
        .attr('width', xScale.bandwidth())
        .attr(
          'height',
          algoInstance && algoInstance.height
            ? algoInstance.height(yScale)
            : d => yScale(d)
        );
    ...
    svg
      .selectAll('rect')
      .data(algoInstance && algoInstance.data ? algoInstance.data() : data, d =>
        d instanceof Object ? d.d : d
      )
      .enter()
      .append('rect')
      .attr(
    ...
    function iterate() {
      ...
      if (
        config.runOffScreen ||
        (boundingRect.top > -svg.node().scrollHeight + 100 &&
        ...
        let rects = svg
          .selectAll('rect')
          .data(algoInstance.data ? algoInstance.data() : data, d =>
            d instanceof Object ? d.d : d
          )
          .attr(
            'class',
            algoInstance.classes ? algoInstance.classes(data) : ''
          );
  ...
  const hslScale = d => {
    return d3.hsl(
      ((typeof d === 'object' ? d.d : d) / config.bars) * 270,
      1,
      0.5
    );
  };
    ...
    return {
      ...
      classes() {
        return ({ d }) => {
          ...
          const to = pass % 2 === 0 ? data2 : data;
          if (
            Math.max(data.length, data2.length) === 2 &&
            to[0] &&
            to[0].indexOf(d) !== -1
          ) {
            return 'completed';
    ...
    return {
      iterate(data) {
        if (!sorting) {
          ...
          pivot =
            sorting[0] + Math.floor((sorting[1] - sorting[0]) * Math.random());
    ...
    return {
      iterate(data) {
        ...
        if (data[actualTest] < data[actualTest - gap]) {
          [data[actualTest], data[actualTest - gap]] = [
            data[actualTest - gap],
            data[actualTest]
          ];
