...
Sometimes you or a computer needs to sort a list of items—say, a list of names into alphabetical order, or a list of objects into order of length. To sort this list, especially if it is a large list, you need some kind of process: for example, if you were sorting a list of names into alphabetic order, you could put all the names beginning with A together, then all the Bs together, and then you could sort them by the second letter: all the AAs together, then all the ABs together, until they're sorted.

Algorithms used to sort a list of items are called sorting algorithms, and in this article I'm going to explain and visualise a few of the most well known sorting algorithms. Each section will contain one sorting algorithm, a description of the algorithm, and then a visualisation of the algorithm, sorting different length bars (seen below) from shortest to longest, powered by [D3.js]. There are some controls to the bottom right of the screen: it might help if you adjust the speed of the visualisation a bit.
...
Bubble sort is one of the easier sorting algorithms to understand, as it is one of the simplest. It is also one of the slowest to run (although I won't really be talking about algorithm speed in this article).

It works by iterating through the list from the beginning, comparing the current item to the next item, and moving the longest of the two up the list. When it gets to the end of the list, the current item is marked as sorted and the iteration begins again from the beginning of the array.
...
Selection sort works by iterating through the list, finding the smallest unsorted item. Once it has found it, it swaps the item with the left-most unsorted item, marks it as sorted, and starts the search again.
...
All the other algorithms in this article are *in-place* algorithms, meaning that they work within the array, swapping elements. Merge sort is not an in-place algorithm, and uses two arrays.

Merge sort works by diving the array into two parts, running merge sort on the two parts so that the individual parts are order, and then combining them again in order, taking the smallest element from the two buckets and adding it to a new array.
...
An algorithm which divides the problem into smaller problems and solves them recursively is known as a divide and conquer algorithm.

----

## Shell sort

Shell sort is a variation on insertion sort, which was explained above. It's a different in that it runs through the list multiple times, and starts by comparing elements that are far apart from each other—the distance between them being known as the "gap".
...
Once we've iterated through the list, we choose another gap: in this example, to keep it simple, I'm just halving and then flooring the gap (so if it was 15, it would become 7). We iterate through the list again, swapping elements when necessary, and if they're swapped we compare them with the element `gap` distance _below_ them so see if a swap is necessary: effectively, we're running insertion sort on a subset of the list where the subset is every `gap` elements of the list.
...
The algorithm works by choosing a pivot element in the list it wants to sort—in this case, by taking a random element—and then setting `l` and `r` variables to the top and bottom points of the list. Then, `l` and `r` move towards the pivot, swapping around if `l` is larger than `r` to ensure that elements on the left of the pivot are always smaller than the pivot. When `l` or `r` gets the pivot, the pivot starts moving too, and when they are all in the same place, it means that the pivot is now in the correct position: everything to the left of it is smaller than it, and everything to the right of it is larger than it.
...
The algorithm works in two stages: in the first stage, it takes unprocessed items one at a time, and adds them one at the time to a list representation of a heap—a tree structure where the parent nodes are always bigger than the child nodes. [This video on heaps][xoax heaps] is a good watch, if you're unfamiliar with the concept.

In the second stage, once the list has become a heap, the largest element—the first element in the list, now—is repeatedly swapped with the last unsorted item in the list. It is then marked as completed, and the item that was just swapped onto the top of the heap repeatedly swapped with its smallest child until the heap is valid again.
...
And, just for fun, here's bogosort. Bogosort works by repeatedly shuffling the array randomly until it is sorting. It can take an infinitely long amount of time to complete: in fact, if we have a list of 20 items and we shuffle it 1,000 times a second, it will take an average of 77 billion years to complete.
